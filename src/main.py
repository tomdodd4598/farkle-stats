...
def dice_perms(n):
    return (np.array(x) for x in itertools.combinations_with_replacement(range1(SIDES), n))
...
def init_scores():
    for n in range1(DICE):
        ...
        for roll in dice_perms(n):
            ...
            prob = Fraction(perm_count(roll), div)
            score_data[roll.tobytes()] = scores, prob
...
def best_score_info(t, roll):
    ...
    roll_bytes = roll.tobytes()
    scores, prob = score_data[roll_bytes]
    ...
    if cutoff_total > 0:
        best_score_cache[(t, roll_bytes)] = result
    return result
...
def get_config():
    return DICE, SIDES, min_total, target_total, farkle_penalty, [(x.tobytes(), y) for x, y in combo_data]


def is_valid_cache():
    if os.path.isfile('data/config'):
        with open('data/config', 'rb') as file:
            if get_config() == pickle.load(file):
                return os.path.isfile('data/roll_ev_cache') and os.path.isfile('data/best_score_cache')
    return False


def load_caches():
    with open('data/roll_ev_cache', 'rb') as file:
        global roll_ev_cache
        roll_ev_cache = pickle.load(file)

    with open('data/best_score_cache', 'rb') as file:
        global best_score_cache
        best_score_cache = pickle.load(file)


def write_caches():
    with open('data/roll_ev_cache', 'wb') as file:
        pickle.dump(roll_ev_cache, file)

    with open('data/best_score_cache', 'wb') as file:
        pickle.dump(best_score_cache, file)


def write_config():
    with open('data/config', 'wb') as file:
        pickle.dump(get_config(), file)
