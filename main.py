...
def get_config():
    return DICE, SIDES, min_total, target_total, farkle_penalty, [(tuple(x), y) for x, y in combo_data]


def is_valid_cache():
    if os.path.isfile('cache_cfg'):
        with open('cache_cfg', 'rb') as file:
...
def load_caches():
    with open('cache', 'rb') as file:
        global roll_ev_cache, best_score_cache
        roll_ev_cache, best_score_cache = pickle.load(file)


def write_caches():
    with open('cache', 'wb') as file:
        pickle.dump((roll_ev_cache, best_score_cache), file)


def write_config():
    with open('cache_cfg', 'wb') as file:
